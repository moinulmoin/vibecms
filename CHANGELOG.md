...
- Marketing site rebuilt on tokens with honest dev-tool visuals (real MCP/API code samples), canonical copy, accessible structure, and no fabricated screenshots.
- Dashboard redesigned on a shared `AppFrame`/`DashboardShell`/`OnboardingFrame` with lucide icons, mobile nav, skip link, and consistent spacing.
