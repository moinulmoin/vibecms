...
function CodeBlock({ name, hint, code }: { name: string; hint: string; code: string }) {
  return (
    <div className="grid gap-2">
      ...
      <pre className="overflow-auto rounded-lg border border-border bg-muted p-3 font-mono text-xs leading-relaxed text-foreground">
        {code}
      </pre>
    </div>
  )
    ...
    <div className="grid gap-4">
      {token ? (
        <Card className="rounded-2xl border-primary/40 shadow-sm">
          ...
          <CardContent className="grid gap-3">
            <pre className="overflow-auto rounded-lg border border-border bg-muted p-4 font-mono text-sm text-foreground">
              {token}
            </pre>
      ...
      <Card className="rounded-2xl border-border shadow-sm">
        ...
        <CardContent className="grid gap-3">
          <div className="flex flex-wrap items-center gap-3">
            <code className="rounded-lg border border-border bg-muted px-3 py-2 font-mono text-sm text-foreground">
              {mcpUrl}
            </code>
      ...
      <Card className="rounded-2xl border-border shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            Paste the snippet for your tool, then start a session. The server teaches the agent the rest.
          </CardDescription>
        </CardHeader>
