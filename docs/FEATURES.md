...
- OTP send rate limit: 5/hour per email (D1, fail-closed when the durable budget cannot be checked).
...
### Editor (dashboard)
- Markdown remains canonical. CodeMirror 6 provides an exact source editor; BlockNote provides a visual canvas only when its adapter can round-trip the document without changing meaningful Markdown.
- Write / Source / Preview / Settings modes; Preview uses the exact public renderer and active site theme.
- Slash commands, search/history, direct image upload and media-library insert; cover-image selector; SEO + tags fields.
- Presentation panel (layout + TOC, constrained to the active theme preset).
- Exact-version autosave/concurrency checks, unsaved-changes guard, and version-history sheet (view / diff vs current / restore).

### Media
- Image upload (jpeg/png/webp/gif, <=10 MB) to R2, served at `/media-assets/<id>` (immutable cache).
- List / delete; delete blocked (409) when the asset is a post cover or site social image, and agent deletion requires the dedicated `assets:delete` scope.
...
- Per-post OG/meta/canonical; `noindex` for unpaid blogs.
...
- REST API `/api/v1` with 18 OpenAPI operations covering the same capabilities (`posts.search` maps to the posts-list search query) + public `GET /api/v1/openapi.json` and Scalar docs at `/api/v1/docs`.
- CLI `@vibecms/cli` (login/whoami/site/posts/assets/schema, including posts get-by-slug); update, restore, and publish require an expected version.
- Bearer tokens `vc_live_...` (HMAC-hashed, peppered, reveal-once); 9 scopes; 3 presets (Drafter=default / Publisher / Full publisher); destructive media deletion is Full-only; max 10 active tokens.
...
### AutoSEOPilot managed hosting
- Workspace-UUID-keyed, idempotent provision/status/revoke internal API with strict schemas, bounded bodies, correlation IDs, constant-time internal-secret checks, and raw-token redaction.
- Durable credential identity + monotonic generations prevent stale replay/reactivation; raw credentials are generated by AutoSEOPilot and only hashed by VibeCMS.
- Managed sponsorship participates in one effective hosted-entitlement check across publishing, media, indexing, analytics, custom domains, and API quotas without becoming a Polar subscription.
- Revoke disables the managed key and sponsorship without deleting the site, posts, versions, media, activity, domains, or owner access.

### Dashboard
- Overview, Analytics, Posts, Media, Subscribers, Activity, Theme, Settings, Connect, Billing, Setup.
- Connect-an-agent flow with focused snippets for Claude Code, Codex, Cursor, Droid, and generic MCP + a protected-read connection check + approval-first starter prompt.
- Inline alerts via 303 `?ok=`/`?error=`.

### Onboarding
- One visible path: Blog setup -> Choose client -> Connect agent -> First draft -> Review exact version -> Publish proof, with manual writing as a fallback.
...
- Free tier: drafting + agent tokens + 5 published posts (noindex); subscription unlocks more publishes, media, search indexing, custom domains, and reader + AI discovery analytics.
...
- **Google/GitHub OAuth sign-in**: code is wired in `auth.ts` and appears only when the corresponding client credentials are provisioned. Email OTP remains the default.
...
- **Page builder or block-canonical content model**: intentionally skipped. Markdown stays canonical; the visual block canvas is a capability-gated editing adapter, not a second storage format.
