...
if rg -n --hidden --glob '!**/node_modules/**' --glob '!**/.vite/**' --glob '!dist/**' --glob '!apps/web/dist/**' --glob '!pnpm-lock.yaml' --glob '!apps/web/.env' --glob '!scripts/public-release-audit.sh' '(polar_(oat|whs)_[A-Za-z0-9]+|ghp_[A-Za-z0-9_]+|\bsk-[A-Za-z0-9_]+|AIza[0-9A-Za-z_-]+|CLOUDFLARE_API_TOKEN|CF_API_TOKEN|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----)' .; then
